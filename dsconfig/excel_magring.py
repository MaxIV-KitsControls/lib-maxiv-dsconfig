    ...
    if "properties" in row:
        ...
        try:
            for prop in properties.split(";"):
                ...
                prop_dict[name.strip()] = value.strip().replace(",",".") #PJB hack for swedish gnumeric
        except ValueError:
            raise ValueError("could not parse Properties")

    # "Property:xyz" columns
    for col_name, value in row.items():
        print "xx", col_name, value, type(value)
        ...
        if match and value:
            ...
            if isinstance(value, float):    # TODO: numeric values become floats, but what if we only want integers?
                value = str(value)
            values = [v.strip().replace(",",".") for v in value.decode("string-escape").split()] #PJB replace hack for swedish gnumeric
...
def xls_to_dict(xls_filename, pages=None, skip=False):
    ...
    for page in pages:
        ...
        #allows page name like "Servers_magnets"
        if page.split("_")[0] not in ["Servers","Dynamics","ParamConfig"]:
            continue
