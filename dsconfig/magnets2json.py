        ...
        if types_classes.has_key(self.itemType):
            ...
            if parsed:
                ...
                if "CIR" in self.itemName:
                    ...
                    already_added = ["B_I_SP02DIPBD_DIA_TSW1_A","B_I_SP02DIPBD_DIA_TSW2_A"] #can use this to ignore to. e.g. SP02DIPDB doesnt end in a number but isn't a circuit!
                    for key in alarm_dict:
                        #compact name is like _TR1QF_ but some tags are like _TR1QF2_5_
                        #if compactnamecir in key:
                        if (key not in already_added and compactnamecir in key) or (compactnamecir[:-1] in key and key.count("_")>5 and key not in already_added and "F"+num+"_" in key):
                            ...
                            alname = 'TemperatureInterlock'+key.split('_')[-2]+'_'+system+"_"+location+'__'+endname.replace("-","_")
                            alsev = alname+":"+"ALARM"
                            alrec = alname+":"+"HTML"
                ...
                if "MAG" in self.itemName and "CIR" not in self.itemName:
                    ...
                    for key in alarm_dict:
                        if compactname in key and key.count("_")<6:
                            ...
                            if 'TemperatureInterlock' not in self.parameters:
                                ...
                                self.parameters['TemperatureInterlock'].append(pyattname+","+key+","+alarm_dict[key])
                    ...
                    for key in alarm_dict:
                        if compactname_nonum in key or (compactname[:-1] in key and key.count("_")>5 and ("F"+num+"_" in key or "_"+num+"_" in key)):
