...
def delete_server(db, dbdict, servername, serverdata):
    for clsname, devices in serverdata.items():
        delete_devices(db, dbdict, servername, clsname, devices.keys())
    try:
...
def delete_class(db, dbdict, classname):
    props = dbdict.classes[classname].properties.keys()
    if props:
        db.delete_class_property(classname, props)
    attr_props = dbdict.classes[classname].attribute_properties.keys()
    if attr_props:
